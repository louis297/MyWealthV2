...
| [tenant-admins](tenant-admins.md) | review | SystemAdmin | `/users/tenant-admins`; dual-write; last-admin disable allowed; disabled tenant → 400 `disabled`/`tenant` | Caller and gap belong here |
...
Repo master (2026-09-13, `df91929`): identity-auth, the adviser-portal **current** slice (Vite resource, PKCE authorize / `/callback`, session probe via `GET /users/me`, 401 refresh-once, OpenIddict redirect upsert including the Aspire dashboard alias), `GET /currencies`, and [tenants](tenants.md) are in the tree. Currencies Feature Spec is still `review`. Tenants landed through the two-tenant isolation fixture. Next backend slice is [tenant-admins](tenant-admins.md) (`review`; not in the tree yet).
