...
internal static class CustomerHttp
{
    ...
    public static async Task<(Tenant Tenant, User Adviser, TokenResponse Tokens)> SignInTenantAdmin(
        string tenantName = "North Advisory",
        string tenantCode = "north-advisory",
        string email = "tess@north.example",
        string password = "Password1!",
        string adviserEmail = "sam@north.example")
    {
        ...
        var adviser = await TestApp.CreatePersonAsync(
            UserRole.Adviser, "Sam Reed", adviserEmail, "Password1!", tenant.Id);
        var tokens = await FunctionalTestSetup.Oidc.SignInAsync(email, password, tenantCode);
        return (tenant, adviser, tokens);
    }

    public static async Task<(Tenant Tenant, User Adviser, TokenResponse Tokens)> SignInAdviser(
        string tenantName = "North Advisory",
        string tenantCode = "north-advisory",
        string email = "sam@north.example",
        string password = "Password1!")
    {
        var tenant = await TestApp.CreateTenantAsync(tenantName, tenantCode);
        var adviser = await TestApp.CreatePersonAsync(
            UserRole.Adviser, "Sam Reed", email, password, tenant.Id);
        var tokens = await FunctionalTestSetup.Oidc.SignInAsync(email, password, tenantCode);
        return (tenant, adviser, tokens);
    }
