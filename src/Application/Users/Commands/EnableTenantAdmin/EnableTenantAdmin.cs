...
namespace MyWealthV2.Application.Users.Commands.EnableTenantAdmin;

[Authorize(Policy = Policies.TenantAdminsManage)]
public class EnableTenantAdminCommand : IRequest
{
...
public class EnableTenantAdminCommandValidator : AbstractValidator<EnableTenantAdminCommand>
{
    public EnableTenantAdminCommandValidator()
    {
        RuleFor(command => command.RowVersion).NotEmpty();
    }
}

public class EnableTenantAdminCommandHandler(IApplicationDbContext db)
    : IRequestHandler<EnableTenantAdminCommand>
{
    public async Task Handle(EnableTenantAdminCommand request, CancellationToken cancellationToken)
    {
